...
**Casino / gambling platforms**: Bonus activation, wagering requirements, deposit matches, free spins, game demo vs real-money mode, and wallet race conditions are a distinct financial-logic subclass not covered by e-commerce patterns. See `references/casino-gambling-business-logic.md` for the casino-specific endpoint naming conventions (MS-BONUS-BALANCES, FREE-MONEY, internal/casino-*), unauth bonus catalog exposure, bonusId IDOR testing, wager-multiplier bypass, and S3 bucket name disclosure from bonus image URLs.
