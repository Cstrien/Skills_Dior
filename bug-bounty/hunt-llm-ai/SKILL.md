...
## MCP Server Endpoint Reconnaissance (before testing the model)

When the target is a remote MCP (Model Context Protocol) server — a Vercel/Next.js
or similar app serving MCP transports over HTTP — start with the server, not the
model. The `WWW-Authenticate: Bearer ... resource_metadata=...` header on any
protected path reveals the `.well-known/oauth-protected-resource/{path}` convention;
from there, pull `/.well-known/oauth-authorization-server` for the full OAuth
metadata (endpoints, scopes, grant types), test dynamic client registration
(RFC 7591) if `registration_endpoint` is present, and enumerate the transport
surface (`/sse`, `/mcp`, `/api/mcp`, `/messages`).

This is distinct from prompt-injection testing: it's infrastructure/transport
recon that produces the OAuth credentials and endpoint map you need before you can
even send an MCP `initialize` request. Full technique and the proven probe
sequence are in `references/mcp-server-endpoint-recon.md`.
