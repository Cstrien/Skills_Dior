...
**NOTE:** `grpcurl` requires either reflection or a `-protoset` descriptor — it CANNOT call arbitrary methods by path alone. When reflection is off and you don't have `.proto` files, drop down to raw `curl` with hand-crafted gRPC-Web frames. See `references/grpc-web-from-js-reflection-disabled.md` for the complete workflow: extract method names from minified JS bundles, build protobuf frames in Python, and parse gRPC-Web responses. This approach was validated against 104 Anduril Lattice hosts behind Envoy where reflection was disabled but JS contained full `getMethodDesc` exports for 108 gRPC methods. See also `references/grpc-web-auth-boundary-probe.md` for classifying edge-vs-backend errors and a minimal probe script pattern.
