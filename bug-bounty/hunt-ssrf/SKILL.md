...
1. **Map all URL-input parameters** across the target: spider JS files for fetch calls, check all API docs, and parse OpenAPI/Swagger request schemas for fields named `url`, `uri`, `webhook_url`, `jwks_url`, `issuer`, `callback`, `domain`, `host`, or `endpoint`. High-signal API-doc sinks include JWKS registration, webhook configuration, SMTP/email-provider host settings, OAuth/OIDC provider setup, Data API auth config, file-import, link-preview, image-proxy, and redirect features.
