...
### Phase 4 — SPA/PWA client-side redirect sinks

Modern React/Vue/Next/PWA apps often perform redirects entirely in client-side JS, so `curl -I --max-redirs 0` may show nothing even when the browser redirects after login, recovery, or route hydration.

Hunt by grepping downloaded bundles/chunks for parameter-to-location flows:

```bash
# High-signal sinks and sources in frontend bundles
grep -RInE "URLSearchParams\(|\.get\(['\"](referer|redirect|return|next|continue|callback)['\"]\)|window\.location\.href|location\.(assign|replace)|history\.(push|replace)" \
  recon/$TARGET/js/ 2>/dev/null
```
...
ref = params.get("referer")
url = maybeHelper(ref)      // verify helper actually validates origin
window.location.href = url  // sink
```

Validation discipline:

- Do not assume a helper call validates the URL. Resolve/export-trace the helper; it may be analytics, formatting, or query appending rather than allowlisting.
- If the sink fires only after login/signup/password-reset, reproduce in a real browser with a test account: `https://target/login?referer=https://attacker.example` → complete flow → observe final navigation.
- If live HTTP probes are blocked by CDN/WAF, static JS evidence is only a lead. A report needs browser proof or a deterministic local reproduction of the exact SPA bundle executing the redirect.
- Check whether the app normalizes external URLs to same-origin paths, rejects non-HTTP schemes, or defaults to `/` when the param is absent.

### Phase 5 — OAuth Chain Test
```bash
...
### Phase 6 — Server-Side Redirect (SSRF escalation)
```bash
