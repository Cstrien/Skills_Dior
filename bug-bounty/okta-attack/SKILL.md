...
### `/.well-known/okta-organization` — tenant metadata + alternate login portals
This public endpoint returns org metadata including an `alternates` array of **alternate Okta login domains** used for partner/customer-specific branded login experiences. One of the highest-yield unauthenticated Okta recon techniques.

```bash
curl -sk "https://<tenant>.okta.com/.well-known/okta-organization" | python3 -m json.tool
```
Response shape:
```json
{
  ...
  "cell": "ok7",
  "pipeline": "idx",
  "settings": {"analyticsCollectionEnabled": true, "bugReportingEnabled": true, ...},
  "alternates": [
...
```
**What to extract:**
- `id` — Okta org ID (useful for API calls, log correlation)
- `cell` — Okta cell/region (infrastructure mapping)
- `alternates` — alternate login portals; each is a branded Okta subdomain with its own CSP, branding, and potentially different client apps. Test each alternate for:
  - Different OIDC clients with different redirect_uri allowlists
  - Weaker security posture (custom branding often means custom config)
  - Partner-specific scope sets
- Internal infrastructure domains referenced in the main tenant's CSP `connect-src` directive (e.g. `<tenant>-admin.okta.com`, `<tenant>.kerberos.okta.com`, `<tenant>.mtls.okta.com`)

### `okta-jwt.js` script tag — Sign-In Widget config extraction
Login pages that embed the Okta Sign-In Widget load a script tag `src="/okta-jwt.js"` with **configuration attributes** that expose the full OIDC client configuration:
...
|---|---|---|
| `clientId` | Okta OIDC client_id | `0oahbc5fbqwwsNhQN356` |
| `baseUrl` | Okta tenant URL | `https://thetradedesk.okta.com` |
| `issuer` | Okta authorization server | `https://thetradedesk.okta.com/oauth2/ausn7e4mbTyJUOMZR356` |
| `redirectUri` | Callback URL (registered redirect_uri) | `https://auth.thetradedesk.com/Account/LoginCallback` |
| `codeChallenge` | PKCE challenge (confirms PKCE in use) | `SDeLrX4Z3O62UNLzSqCFrW1bzVMqxAE2b-DUFE86WVs` |
| `codeChallengeMethod` | PKCE method | `S256` |
| `state` | OAuth state parameter | `l0TBJldSx9AILO6npcqWsQ` |
| `returnUrl` | Full IdentityServer authorize URL (reveals downstream client_id, scopes, redirect_uri) | `/connect/authorize/callback?client_id=pubdesk&scope=...` |
| `brandName` | Branding confirmation | `The Trade Desk` |
| `isMixedModeEnabled` | Whether mixed-mode auth is on | `False` |

**Key insight:** The `returnUrl` attribute often contains the **full downstream IdentityServer authorize URL** with `client_id`, `scope`, `redirect_uri`, and `response_type` for the calling application — this reveals additional OAuth clients (e.g. `pubdesk`) and their scope sets without needing to trigger a login flow.
