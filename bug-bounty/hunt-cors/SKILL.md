...
**Tracking/identity-pixel variant (often missed):** Third-party identity and
tracking endpoints (e.g. `bh-<vendor>-cdn.<vendor>.com/<product>/v1`, pixel
servers, audience-sync endpoints) sometimes reflect ANY origin + send
`Access-Control-Allow-Credentials: true` AND set cross-domain tracking
cookies (`V=`, `VP=`, `INGRESSCOOKIE=` on `.vendor.com`). These are
exploitable because the main app calls the endpoint with
`credentials: 'include'`, the cookies are on the vendor's root domain
(readable from any origin), and the endpoint may return user-segment or
identity data in the JSON body. Always check for `timing-allow-origin: *`
and `Access-Control-Expose-Headers` listing custom debug headers
(e.g. `X-Ppfp-Dbg`) — these amplify impact by leaking pod names, server
versions, and request fingerprints.
