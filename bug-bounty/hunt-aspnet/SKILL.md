...
## ASP.NET CrossDomainProxy + proxyOptions JS Extraction

Classic ASP.NET Webforms apps that use a `CrossDomainProxy.ashx` handler for cross-domain API calls embed a `proxyOptions` JavaScript object inline in the page HTML (typically on `Signup.aspx`, `AdvertiserSignup.aspx`, `ForgotPassword.aspx`, and other pre-auth pages). This object leaks the **complete backend API service inventory** — every service name, proxy path, and auth token state — without authentication.

**Extraction:**
```bash
# Download any pre-auth .aspx page and grep for proxyOptions
curl -sk -H "User-Agent: Mozilla/5.0" \
  "https://target.example/AccountMgmt/Signup.aspx" | \
  grep -oE 'proxyOptions\s*=\s*\{[^}]+\}'
# Or extract service names directly:
curl -sk "https://target.example/AccountMgmt/Signup.aspx" | \
  grep -oE '(BaseUri|ServiceBaseUri):\s*[^,}]+' | sort -u
```

**What it reveals:**
- Every backend service name (`UserService`, `AccountService`, `SecurityServices`, `AdminAPI`, `Publisher`, `Advertiser`, `AuthorizationService`, `PassCodeService`, `Logger`, etc.)
- The proxy URL pattern (`/AccountMgmt/CrossDomainProxy.ashx?a=<Service>`)
- Whether `useProxy` is `"true"` (all API calls routed through the proxy)
- The `AuthToken` value (usually empty string on pre-auth pages, but sometimes leaks session tokens on authenticated pages)

**Pivot to WCF .svc discovery:** The service names from `proxyOptions` map directly to `UIWebServices/<ServiceName>.svc` paths. Test each one for the unauthenticated-access differential (see next section).

## Unauthenticated WCF .svc Differential

In ASP.NET Webforms apps with `UIWebServices/` directories, most `.svc` files redirect to the login page (302) when accessed without auth. But some — particularly those used by pre-auth flows like signup — return 200 and serve the WCF help page + WSDL without authentication.

**Technique — enumerate all .svc files, classify by auth state:**
```bash
# Build service name list from proxyOptions or JS analysis
for svc in AccountService UserService SecurityServices Publisher Advertiser \
           AdminAPI PassCodeService Logger AuthorizationService; do
  code=$(curl -sk -o /dev/null -w "%{http_code}" \
    -H "User-Agent: Mozilla/5.0" \
    "https://target.example/AccountMgmt/UIWebServices/${svc}.svc")
  # 200 = UNAUTHENTICATED ACCESS (finding!)
  # 302 = redirects to login (auth-gated, expected)
  # 404 = service doesn't exist
  echo "$code ${svc}.svc"
done
```

Any `.svc` returning 200 is an unauthenticated WCF service. Immediately fetch `?singleWsdl` to get the full data contract (operations, parameter types, field names including `Password`, `SecurityAnswer`, etc.).

## WCF Metadata Endpoint Internal Hostname Leak

WCF services expose metadata via three endpoints, and in a load-balanced farm each endpoint may be served by a **different backend node** — leaking multiple internal hostnames from a single service:

```bash
# Each endpoint leaks a DIFFERENT internal hostname:
curl -sk "https://target.example/AccountMgmt/UIWebServices/AccountService.svc" \
  | grep -oE 'http://[a-z0-9.-]+\.corp[^"]*'
# → http://ma2-portal01.pulse.corp  (help page)

curl -sk "https://target.example/AccountMgmt/UIWebServices/AccountService.svc?disco" \
  | grep -oE 'http://[a-z0-9.-]+\.corp[^"]*'
# → http://ma2-portal02.pulse.corp  (discovery document)

curl -sk "https://target.example/AccountMgmt/UIWebServices/AccountService.svc?wsdl" \
  | grep -oE 'http://[a-z0-9.-]+\.corp[^"]*'
# → http://ma2-portal03.pulse.corp  (WSDL import location)
```

This reveals:
- Internal domain name (`.pulse.corp`)
- Server naming convention (`ma2-portal0X`)
- Load-balanced farm topology (3+ nodes)
- QA/staging hostnames (different prefix on demo environments)

## Client-Side Password Crypto Weakness (SHA-256 Truncation + Visible AES Key)

Some ASP.NET Webforms apps implement client-side password hashing before form submission. A weak implementation seen in the wild:

1. `passwordHash = SHA-256(password)` → 64 hex chars
2. **Sends only `passwordHash.substring(0, 19)`** — truncating to 19 chars, reducing from 256 bits to ~95 bits of entropy
3. AES-GCM encrypts the plaintext password using `VIEWSTATEGENERATORCUSTID` (a per-session random 32-char hex key, padded to 32 bytes with spaces) as the AES key
4. The AES key is embedded in the HTML as a hidden form field

**Detection:**
```bash
# Check if password is hashed client-side
curl -sk "https://target.example/AccountMgmt/Login.aspx" | \
  grep -oE 'hashStringSHA256|passwordHash|substring\(0,\s*19\)'
# Check for the visible AES key
curl -sk "https://target.example/AccountMgmt/Login.aspx" | \
  grep -oE 'VIEWSTATEGENERATORCUSTID" value="[^"]*"'
```

**Impact:** The truncation weakens the hash significantly. The AES key is visible in the page source (security through obscurity). An attacker who intercepts the form submission gets a 19-char hash prefix that is much easier to brute-force than a full SHA-256 hash.

See `references/pulsepoint-exchange-aspnet-engagement.md` for the full session walkthrough including proxyOptions extraction, WCF unauth discovery, OpenAPI on auth infrastructure, and CORS analysis.
