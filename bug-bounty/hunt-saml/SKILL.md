...
### ACS Negative-Response Discipline

When testing an ACS endpoint with unsigned, malformed, or attacker-crafted `SAMLResponse` values, app-specific errors such as `302 /login?msg=Internal+server+error` or a custom login failure page only prove that the ACS parsed enough XML to enter SAML handling. They do **not** prove signature bypass, XXE, or account creation/login. Keep probing only if you can show one of:

- a session cookie or OAuth token issued for the forged subject;
- a changed authenticated identity/role after callback;
- OOB XXE/SSRF interaction from a unique payload;
- reflected file/entity content or security-relevant parser differential.

For blind XXE in SAML, use an OOB endpoint (e.g. Collaborator/interactsh) and require DNS/HTTP callback before claiming impact. A generic internal-server-error response with no OOB callback is a negative/inconclusive result.

Do not overstate public SP metadata flags. `AuthnRequestsSigned="false"` is common for SP-initiated SSO and is usually not a vulnerability when `WantAssertionsSigned="true"` and assertion signature/audience checks hold. Treat public SAML metadata, Okta app IDs, and valid login-domain oracle as identity-fabric disclosure/enablers unless chained to auth bypass, token theft, or tenant/user data access.
