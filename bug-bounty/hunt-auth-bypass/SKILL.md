...
**gRPC-web patterns indicating federated auth (SPA → gRPC backend):**
When a React/Vue SPA communicates with a gRPC backend through Envoy/gRPC-Web, the auth service methods may be callable without authentication. Look for:
- JS bundle strings: `GetSSOURL`, `GetSPMetadata`, `GetPrimaryIDP`, `ListIDPs`, `LoginPassword`
- gRPC package names: `*.auth.v2.*`, `*.auth.v3.*`
- `getMethodDesc` exports in minified JS revealing all service method names
- gRPC-Web POST to `/package.Service/Method` returning `grpc-status:0` without any auth header

These methods can leak: Okta/SAML app IDs, SAML entity IDs, SP metadata + X509 certificates, ACS URLs, and serve as a valid-domain oracle (e.g., `*@company.com` returns SAML redirect while `*@gmail.com` does not). See `hunt-grpc` → `references/grpc-web-from-js-reflection-disabled.md` for the extraction workflow.
